# -*- coding: utf-8 -*-
"""DeerDataModifier.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1tkhFuKMqqi4ffqccJ676s1YSjga41hvj
...
This code is not necessary for the deer binary model now, as the images have already been added to the folders. Any array storage and image manipulation after the images are stored to the folders is irrelevant for that project. However, the code is saved in case it can be repurposed.
...
"""The np.load function is updated to set allow_pickle = true, which allows the code to save and load arrays and models. However, the old version of np.load is stored in case we need to reset to the old np.load."""

np_load_old = np.load
np.load = lambda *a,**k: np_load_old(*a, allow_pickle=True, **k)

"""Instantiating the arrays we will need later in the program"""
...
This would call the method *canOpener* on deer.csv and notDeer.csv, looping through these and added the images to the folders labelled deer_train, deer_test, notDeer_train and notDeer_test
